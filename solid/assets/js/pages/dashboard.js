...
if (user) {
  ...
  /* 削除ボタン列：管理者・発注者（管理者でもモデラーでもない一般会員）に表示。
    ...
    ps.forEach(p => {
      ...
      tr.innerHTML = `
        ...
        <td style="min-width:160px;max-width:260px;">
          ...
          ${(isModeler(user) || isSuperAdmin(user) || isOperator(user)) ? `<span class="project-company-label" style="font-size:11px;color:var(--muted);">${companyName}</span>` : ''}
        </td>

...
- **`justify-content:flex-end` の1行に押し込むと、はみ出しは画面の「左」へ流れる**。
  scrollWidth には出ず、スクロールでも追えない。実際にダッシュボードの下部バーは
  1280px 幅でキャンセルボタンが切れ、`0件選択中` が幅1文字まで潰れて縦書きになっていた。
  受け皿は `flex-wrap: wrap`（行を増やす）＋ 件数側に `flex:0 0 auto; white-space:nowrap`
- **日本語の短いラベルは flex-shrink で1文字幅まで潰れる**。`.btn` のように
  `white-space:nowrap` があるものは min-content が効いて潰れないが、素の span は潰れる。
  「1.2 MB」が「1.2 / MB」に割れるのも同じ原因（ファイルカードの日付/サイズ行）
- **`overflow:hidden` の中のボタンは、はみ出すと押せないまま消える**。
  ファイルカードのダウンロード、ファイル詳細ツールバーの印刷・削除が実際にそうなっていた。
  折り返す（カード）か、帯の中で横スクロールさせる（ツールバー）
- **境目は実測で決める**。`responsive-e2e.js` は1行に収まっている余裕をpxで出す。
  ダッシュボードの下部バーはラベル全文だと1400pxでも余裕27pxしか無く、フォントが違えば
  折り返す ⇒ 短縮ラベルの境目を1499pxにした
- **`position:fixed` の要素は `offsetParent` が null**。「表示されているか」の判定に使うと
  ボトムナビの重なりチェックが永久にPASSする（実際に取りこぼしていた）。
  `getComputedStyle(el).display` で見る
- Knowl のスマホ高さが `100svh - 56px - 60px` とヘッダー実寸(72px)でズレており、
  入力欄と「新しい会話」が16pxボトムナビの下に潜っていた。高さの決め打ちは `var(--header-h)` へ
